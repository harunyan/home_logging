    ...
    function handlePanUpdate(minTime, maxTime) {
      ...
      checkAndFetchOlderData(minTime);
    }

    let isFetchingOlderData = false;
    let hasReachedBeginningOfData = false;

    async function checkAndFetchOlderData(neededMinTime) {
      if (isFetchingOlderData || hasReachedBeginningOfData || rawEventsData.length === 0 || !neededMinTime) return;

      let oldestTime = Infinity;
      let oldestId = Infinity;
      rawEventsData.forEach(e => {
        const t = new Date(e.timestamp).getTime();
        if (!isNaN(t) && t < oldestTime) oldestTime = t;
        if (e.id && e.id < oldestId) oldestId = e.id;
      });

      const duration = (currentRangeMinutes > 0 ? currentRangeMinutes : 1440) * 60 * 1000;
      const marginMs = duration * 0.3;
      if (neededMinTime - marginMs <= oldestTime) {
        await fetchOlderData(oldestId, oldestTime);
      }
    }

    async function fetchOlderData(beforeId, beforeTimeMs) {
      if (isFetchingOlderData || hasReachedBeginningOfData) return;
      isFetchingOlderData = true;

      const liveIndicator = document.getElementById('live-indicator');
      const prevIndicatorText = liveIndicator ? liveIndicator.textContent : '';
      if (liveIndicator) {
        liveIndicator.textContent = '⏳ 過去データ取得中...';
      }

      try {
        const url = `/api/v1/events?limit=4000&before_id=${beforeId}`;
        const res = await fetch(url);
        const data = await res.json();

        if (data.events && data.events.length > 0) {
          const existingIds = new Set(rawEventsData.map(e => e.id));
          const newEvents = data.events.filter(e => !existingIds.has(e.id));

          if (newEvents.length === 0) {
            hasReachedBeginningOfData = true;
          } else {
            rawEventsData = [...rawEventsData, ...newEvents];
        ...
        console.error('Fetch older data error:', err);
      } finally {
        isFetchingOlderData = false;
        if (liveIndicator) {
          liveIndicator.textContent = prevIndicatorText || (timeOffsetMs <= 60000 ? '● LIVE' : '⏱️ 過去');
        }
      }
    }
    ...
    function panTimeStep(direction) {
      ...
      renderChart();
      const duration = (currentRangeMinutes > 0 ? currentRangeMinutes : 1440) * 60 * 1000;
      const minTime = Date.now() - timeOffsetMs - duration;
      checkAndFetchOlderData(minTime);
    }
    ...
    let cachedDailyMealsTiles = [];

    function updateMealCard(s) {
      if (!s) return;
      ...
      if (s.daily_meals_tiles) {
        cachedDailyMealsTiles = s.daily_meals_tiles;
      }
    }

    function openDailyMealsModal() {
      const modal = document.getElementById('meals-modal');
      if (modal) {
        modal.classList.add('open');
        renderMealsTilesGrid();
      }
    }

    function closeDailyMealsModal(e) {
      if (e && e.target && e.target.closest && e.target.closest('.modal-content') && !e.target.classList.contains('modal-close-btn')) {
        return;
      }
      const modal = document.getElementById('meals-modal');
    ...
    function renderMealsTilesGrid() {
      const grid = document.getElementById('meals-tiles-grid');
      if (!grid) return;

      if (!cachedDailyMealsTiles || cachedDailyMealsTiles.length === 0) {
        grid.innerHTML = '<div style="grid-column:1/-1; text-align:center; padding:30px; color:var(--text-muted);">食事履歴データを読み込み中...</div>';
        return;
      }

      grid.innerHTML = cachedDailyMealsTiles.map(tile => {
        const isTodayClass = tile.is_today ? 'is-today' : '';
        const todayBadge = tile.is_today ? '<span class="tile-badge-today">今日</span>' : '';

        let sessionsHtml = '';
        if (tile.meal_sessions && tile.meal_sessions.length > 0) {
          sessionsHtml = tile.meal_sessions.map((ms, idx) => `
            <div class="tile-session-row">
              <span>#${idx + 1} ${ms.time}</span>
              <span class="tile-session-eaten">${Number(ms.eaten_g).toFixed(1)}g</span>
            </div>
          `).join('');
        } else {
          sessionsHtml = '<div style="text-align:center; font-size:0.72rem; color:var(--text-muted); padding:4px;">食事記録なし</div>';
        }

        return `
          <div class="meal-day-tile ${isTodayClass}">
            <div class="tile-header">
              <span class="tile-date">📅 ${escapeHtml(tile.display_date)}</span>
              ${todayBadge}
            </div>
            <div class="tile-stats">
              <div class="tile-count">${tile.meals_count}<span style="font-size:0.75rem; font-weight:normal; margin-left:2px;">回</span></div>
              <div class="tile-eaten">${Number(tile.food_eaten_g).toFixed(1)} g</div>
            </div>
            <div class="tile-sessions-list">
              ${sessionsHtml}
            </div>
          </div>
        `;
