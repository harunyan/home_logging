    ...
    let selectedMealDate = new Date().toLocaleDateString('sv-SE'); // YYYY-MM-DD

    function updateMealCard(s) {
      if (!s) return;
      const todayStr = new Date().toLocaleDateString('sv-SE');
      const targetDate = s.target_date || todayStr;
      selectedMealDate = targetDate;

      const isToday = (targetDate === todayStr);
      const titleElem = document.getElementById('meal-card-title');
      if (titleElem) {
        titleElem.textContent = isToday ? '本日の食事' : `${targetDate.slice(5).replace('-', '/')} の食事`;
      }
      ...
      const btnNext = document.getElementById('btn-meal-next');
      if (btnNext) {
      ...
      const container = document.getElementById('meal-sessions-container');
      if (container) {
        if (s.meal_sessions && s.meal_sessions.length > 0) {
          container.innerHTML = s.meal_sessions.map((ms, idx) => `
            <span class="meal-session-chip" title="測定: ${escapeHtml(ms.timestamp || '')}">
              <span style="opacity:0.75;">#${idx + 1}</span> <span>${ms.time}</span> <strong style="color:var(--accent-orange);">${ms.eaten_g.toFixed(1)}g</strong>
            </span>
          `).join('');
        } else {
          container.innerHTML = '<span style="font-size:0.68rem; color:var(--text-muted); padding:2px 4px;">食事記録なし</span>';
        }
      }
    }

    async function fetchMealStatsForDate(dateStr) {
      selectedMealDate = dateStr;
      const picker = document.getElementById('meal-date-picker');
      if (picker) picker.value = dateStr;

      try {
        const res = await fetch(`/api/v1/summary?date=${dateStr}`);
        const data = await res.json();
        if (data) {
          updateMealCard(data);
        }
    ...
    function changeMealDate(daysDelta) {
      const parts = selectedMealDate.split('-').map(Number);
      const d = new Date(parts[0], parts[1] - 1, parts[2]);
      d.setDate(d.getDate() + daysDelta);
      const nextDateStr = d.toLocaleDateString('sv-SE');
      const todayStr = new Date().toLocaleDateString('sv-SE');
      if (nextDateStr > todayStr) return;
      fetchMealStatsForDate(nextDateStr);
    }

    function onMealDateChange(val) {
      if (val) fetchMealStatsForDate(val);
    }

    function resetMealDateToToday() {
      const todayStr = new Date().toLocaleDateString('sv-SE');
      fetchMealStatsForDate(todayStr);
    }
